...
class Functions{
	...
	{
		$this->query = "
			SELECT admin_type FROM admin_table
	...
	function master_access() //prevent access without login
	{
		if(isset($_SESSION['admin_id']))
		{
			$admin_type = $this->get_admin_type($_SESSION['admin_id']);

			if($admin_type != 'master'){   //if sub_master
				$this->redirect('../sub_master/');
			}
	...
	function sub_master_access()
	{
		if(isset($_SESSION['admin_id']))
		{
			$admin_type = $this->get_admin_type($_SESSION['admin_id']);
			if($admin_type != 'sub_master') //if master
			{   
	...
	function Is_exam_completed($online_exam_id)
	{
		date_default_timezone_set("Asia/Kolkata");
		$current_datetime = date("Y-m-d") . ' ' . date("H:i:s", STRTOTIME(date('h:i:sa')));
		...
		$this->query = "
		SELECT online_exam_datetime, online_exam_duration FROM online_exam_table 
		WHERE online_exam_id = '$online_exam_id'
		";
		$result = $this->query_result();
	...
	function Get_exam_question_limit($exam_id) //for getting limit of questions
	{
		$this->query = "
	...
	function Get_exam_total_question($exam_id) //for getting already added questions
	{
		$this->query = "
