...
/*for storing current date and time*/
...
if(isset($_POST['page']))
{
	if($_POST['page'] == 'register')
	{
		...
		if($_POST['action'] == 'register')
		{
			...
			<p>This is a verification eMail, please click the link to verify your eMail address by clicking this <a href="'.$exam->home_page.'verify_email.php?type=admin&code='.$admin_verification_code.'" target="_blank"><b>link</b></a>.</p>
	...
	if($_POST['page'] == 'exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = $row['exam_enroll_code'];
				...
				{
					$edit_button = '
					<button type="button" name="edit" class="btn btn-primary btn-sm edit" id="'.$row['online_exam_id'].'">Edit Exam</button>
					';

					$delete_button = '<button type="button" name="delete" class="btn btn-danger btn-sm delete mt-2" id="'.$row['online_exam_id'].'">Delete Exam</button>';

				}
				else
				{
					if($exam->Is_exam_completed($row["online_exam_id"]))
					{
						$result_button = '
						<a href="exam_result.php?code='.$row["online_exam_code"].'" class="btn btn-dark btn-sm">View</a>
						';

						$delete_button = '<button type="button" name="delete" class="btn btn-danger btn-sm delete" id="'.$row['online_exam_id'].'">Delete Exam</button>';
					}
				}
				...
				{
					$question_button = '
					<button type="button" name="add_question" class="btn btn-info btn-sm add_question" id="'.$row['online_exam_id'].'">Add Questions</button>
					<a href="view_questions.php?code='.$row['online_exam_code'].'" class="btn btn-warning btn-sm mt-2">View Questions</a>
					';
				...
				{
					$question_button = '
					<a href="view_questions.php?code='.$row['online_exam_code'].'" class="btn btn-warning btn-sm">View Questions</a>
					';
				}

				$sub_array[] = $status;
				$sub_array[] = $question_button;
				$sub_array[] = '<a href="user_enroll.php?code='.$row['online_exam_code'].'" class="btn btn-secondary btn-sm">View Students</a>';
		...
		if($_POST['action'] == 'Create')
		{
			$exam->data = array(
				...
				':exam_enroll_code'     =>  $_SESSION['admin_id'] . date("His")
			);

			$exam->query = "
			INSERT INTO online_exam_table 
			(admin_id, online_exam_title, online_exam_datetime, online_exam_duration, total_question, marks_per_right_answer, marks_per_wrong_answer, online_exam_created_on, online_exam_status, online_exam_code, exam_enroll_code) 
			VALUES (:admin_id, :online_exam_title, :online_exam_datetime, :online_exam_duration, :total_question, :marks_per_right_answer, :marks_per_wrong_answer, :online_exam_created_on, :online_exam_status, :online_exam_code, :exam_enroll_code)
			";
	...
	if($_POST['page'] == 'users_list')
	{
		if($_POST['action'] == 'fetch') //table
		{
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = '<button type="button" id="'.$row["user_id"].'" name="view_detail" class="btn btn-danger btn-sm details">View Details</button>';
				$data[] = $sub_array;
			}
		...
		if($_POST['action'] == 'fetch_data') //popup
		{
			$exam->query = "
			SELECT * FROM user_table 
	...
	if($_POST['page'] == 'examiners_list')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			{
				$exam->query .= 'ORDER BY admin_id DESC ';
			}
